...
function getStatClass(count: number, warn: number, alert: number): string {
  if (count >= alert) return 'alert'
...
export function StatsPanel({ stats }: Props) {
  ...
  return (
    <div className="panel stats-panel">
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 8, flex: 1, minHeight: 0 }}>
        <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          <div className="hall-list">
            {stats.hallStats.map(h => {
              const cls = getStatClass(h.agentCount, 15, 30)
              return (
